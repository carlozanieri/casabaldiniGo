...
func GetSliders(codice string) ([]Slider, error) {
	if codice != "index" {
		rows, err := db.DB.Query("SELECT id, codice, codice2, img, titolo, caption, link, testo FROM beb_slider WHERE codice2=?  order by codice2", codice)
		if err != nil {
			return nil, err
		}
		defer rows.Close()
		...
		rows, err := db.DB.Query("SELECT id, codice, codice2, img, titolo, caption, link, testo FROM beb_slider WHERE codice=?  order by codice2", codice)
		if err != nil {
			return nil, err
		}
		defer rows.Close()
...
func LecamereHandler(w http.ResponseWriter, r *http.Request) {
	type PageData struct {
		Title        string
		Items        []string
		Year         int
...
func LasalaHandler(w http.ResponseWriter, r *http.Request) {
	type PageData struct {
		Title        string
		Items        []string
		Year         int
...
func IlpaeseHandler(w http.ResponseWriter, r *http.Request) {
	type PageData struct {
		Title        string
		Items        []string
		Year         int
